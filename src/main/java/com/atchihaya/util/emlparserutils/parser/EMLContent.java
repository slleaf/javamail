...
import com.atchihaya.util.emlparserutils.parser.content.Body;
import com.atchihaya.util.emlparserutils.parser.content.ImageBody;
import com.atchihaya.util.emlparserutils.parser.content.TextBody;
import com.atchihaya.util.emlparserutils.utils.EMLHtmlStringUtils;
...
//                value = BodyUtils.translateString(arr[0]) + BodyUtils.translateString(arr[1]);
                if (arr.length >= 2) {
                    ...
                    // 处理数组长度不足2的情况，例如只有一个元素的情况
                    value = BodyUtils.translateString(arr[0]);
                }
            }
        }
